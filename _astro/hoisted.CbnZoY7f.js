import{b,p as y,f as l,e,r as w,t as E,l as L}from"./hoisted.DVhLwWGR.js";const m=document.getElementById("detalhe"),u=document.getElementById("carregando"),g=document.getElementById("nao-encontrado"),p=document.getElementById("trilha-nome");function j(){return""}function q(t){const{atual:s,de:x,promo:d}=y(t),c=l(s),r=l(x),n=!t.disponivel,i=t.imagens[0];p&&(p.textContent=t.nome),document.title=`${t.nome} | Lojinha da Miih`;const v=t.imagens.length>1?`<div class="mt-3 grid grid-cols-4 gap-2">${t.imagens.map((a,$)=>`<button type="button" class="miniatura overflow-hidden rounded-lg border border-fundo-borda bg-fundo-claro focus-visible:border-marca" data-src="${e(a.src)}" data-alt="${e(a.alt)}" aria-label="Ver imagem ${$+1}"><img src="${e(a.src)}" alt="${e(a.alt)}" width="160" height="160" loading="lazy" decoding="async" class="aspect-square w-full object-cover" /></button>`).join("")}</div>`:"",h=c?`<div class="flex items-baseline gap-3">
      ...
      <div class="grid gap-8 md:grid-cols-2">
        ...
        <section>
          ...
          <p class="mt-5 whitespace-pre-line leading-relaxed text-texto-suave">${e(E(t.descricao))}</p>

          <div class="mt-6">
            <a href="${e(L(t))}" target="_blank" rel="noopener" class="btn-marca">
