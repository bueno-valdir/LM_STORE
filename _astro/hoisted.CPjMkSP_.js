const g={whatsapp:"5515998234551",pocketbaseUrl:"https://painel.lmstore.veraxlegalops.com.br"},d="/LM_STORE";function f(t){if(/^([a-z]+:)?\/\//i.test(t)||t.startsWith("#"))return t;const e=d.endsWith("/")?d.slice(0,-1):d,o=t.startsWith("/")?t:`/${t}`;return`${e}${o}`}const i=g.pocketbaseUrl.replace(/\/$/,""),w={maquiagem:"Maquiagem",skincare:"Skincare","kits-presentes":"Kits e Presentes"};function x(t){if(t===""||t===null||t===void 0)return null;const e=Number(t);return Number.isFinite(e)?e:null}function s(t){return(t||"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}function q(t){return(t||"").replace(/<\s*br\s*\/?>/gi,`
`).replace(/<\/(p|div|li|h[1-6])>/gi,`
`).replace(/<[^>]+>/g,"").replace(/&nbsp;/gi," ").replace(/\n{3,}/g,`

`).trim()}function h(t){const o=(Array.isArray(t.imagens)?t.imagens:t.imagens?[t.imagens]:[]).map(a=>({src:`${i}/api/files/produtos/${t.id}/${a}`,alt:t.nome||"Produto"}));return{id:t.id,nome:t.nome||"",marca:t.marca||"",categoria:t.categoria||t.select||"",descricao:t.descricao||"",preco:x(t.preco),precoPromocional:x(t.preco_promocional),tom:t.tom||null,volume:t.volume||null,imagens:o.length?o:[{src:f("/images/products/placeholder-1.svg"),alt:t.nome||"Produto"}],disponivel:!!t.disponivel,destaque:!!t.destaque,promocao:!!t.promocao}}async function y(){if(!i)return[];const t=await fetch(`${i}/api/collections/produtos/records?perPage=200&sort=-created`);if(!t.ok)throw new Error("HTTP "+t.status);return((await t.json()).items||[]).map(h)}async function k(t){if(!i||!t)return null;const e=await fetch(`${i}/api/collections/produtos/records/${encodeURIComponent(t)}`);return e.ok?h(await e.json()):null}function v(t){return w[t]||t||""}function m(t){return t==null?null:t.toLocaleString("pt-BR",{style:"currency",currency:"BRL"})}function $(t){return typeof t.precoPromocional=="number"&&t.precoPromocional>0?{atual:t.precoPromocional,de:t.preco,promo:!0}:{atual:t.preco,de:null,promo:!1}}function j(t){const e=[`Olá! Tenho interesse no produto: ${t.nome} (${t.id})`];t.tom&&e.push(`, tom ${t.tom}`);const{atual:o}=$(t),a=m(o);return a&&e.push(` - ${a}`),`https://wa.me/${g.whatsapp}?text=${encodeURIComponent(e.join(""))}`}function T(t,e,o,a){if(!a.length)return"";const c=a.map(r=>`<div class="min-w-[46%] snap-start sm:min-w-[31%] lg:min-w-[23%]">${P(r)}</div>`).join(""),n=(r,l,p)=>`<button type="button" class="carrossel-${r} grid h-9 w-9 place-items-center rounded-full border border-fundo-borda text-texto transition-colors hover:border-marca hover:text-marca" aria-label="${l}">${p}</button>`;return`
  ...
  </section>`}function P(t){const{atual:e,de:o,promo:a}=$(t),c=m(e),n=m(o),r=!t.disponivel,l=t.imagens[0],p=`
