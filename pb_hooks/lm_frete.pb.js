...
//
// ENTREGAS (Melhor Envio) - hook do servidor (PocketBase).
// =========================================================
// Roda NO SERVIDOR, nunca no navegador. O navegador so manda { id, cep, qty }.
//
// TRAVAS de seguranca:
//  1) Token do Melhor Envio lido de variavel de ambiente (MELHOR_ENVIO_TOKEN).
...
//  5) So devolve as opcoes dos Correios (PAC/SEDEX), sem dados sensiveis.
//
...
routerAdd('POST', '/lm/frete', (e) => {
  ...
  const peso = produto.getFloat('peso') || DIM_PADRAO.peso;
  ...
  const valor = produto.getFloat('preco') || 0;
  ...
  const base =
    env('MELHOR_ENVIO_AMBIENTE', 'sandbox') === 'producao'
      ? 'https://melhorenvio.com.br'
      : 'https://sandbox.melhorenvio.com.br';
  // O Melhor Envio exige um User-Agent identificando a aplicacao + e-mail.
  const userAgent = env('MELHOR_ENVIO_UA', 'Lojinha da Miih (contato@lojinhadamiih.com.br)');
  ...
  for (let i = 0; i < lista.length; i++) {
    ...
    const empresa = (s.company && s.company.name) || '';
    // TRAVA 5: so Correios.
